...
public class ParallaxBackground_test03 : MonoBehaviour
{
    [SerializeField]
    private Transform cameraTransform;      //카메라의 trasnfrom 컴포넌트

    private Vector3 cameraStartPosition;    // 게임을 시작했을 때 카메라의 시작 위치
    private float distance;                 // cameraStartPosition으로 부터 현재 카메라 까지의 x 이동거리
    ...
    [SerializeField]
    [Range(0.01f, 1.0f)]
    private float parallaxSpeed;  // layerMoveSpeed에 곱해서 사용하는 스크롤 이동 속도


    void Awake()
    {
        //게임 시작할 때 카메라의 위치 저장( 이동 거리 계산용)
        ...
        // 각 배경의 material과 이동속돌르 저장할 배열 선언
        materials = new Material[backgroundCount];
        layerMoveSpeed = new float[backgroundCount];


        // GetChild() 메소드를 호출해 자식으로 있는 배경 정보들을 불려온다.
        for(int i = 0; i < backgroundCount; i++)
        {
            backgrounds[i] = transform.GetChild(i).gameObject;
            materials[i]= backgrounds[i].GetComponent<Renderer>().material; 
            //getcomponent<material> 이 아니라 랜더러를 거치는 이유가 뭐지?
        }

        //레이어(카메라와의 z거리 기준)별로 이동 속도 설정
